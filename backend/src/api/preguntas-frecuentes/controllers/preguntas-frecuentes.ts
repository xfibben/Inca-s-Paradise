...
const UID = 'api::preguntas-frecuentes.preguntas-frecuentes';
  ...
  async exportCsv(ctx) {
    ...
    const getEntry = async (localeCode: string | null): Promise<any> => {
      ...
      return await strapi.documents(UID).findFirst({
        ...
        populate: {
          sections: {
            fields: ['pregunta', 'respuesta'] as any,
          } as any,
        } as any,
      });
    };
    ...
    const row = [
      ...
      en?.metaDescription ?? '',
      pt?.metaDescription ?? '',
      buildSectionsJson(Array.isArray(es?.sections) ? es.sections : []),
      buildSectionsJson(Array.isArray(en?.sections) ? en.sections : []),
      buildSectionsJson(Array.isArray(pt?.sections) ? pt.sections : []),
      es?.publishedAt ? 'True' : 'False',
    ];

    const csv = [headers.map(escapeCsvValue).join(','), row.map(escapeCsvValue).join(',')].join('\n');
    ctx.set('Content-Type', 'text/csv; charset=utf-8');
    ctx.set('Content-Disposition', `attachment; filename="preguntas-frecuentes-odoo-${status}.csv"`);
    ctx.body = csv;
  },
}));
