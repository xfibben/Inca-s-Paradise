...
class IncasReservaHotelLinea(models.Model):
    _name = "incas.reserva.hotel.linea"
    _description = "Línea de hotel de reserva"
    _order = "sequence, id"
    ...
    hotel_tarifa_id = fields.Many2one("incas.hotel.tarifa", string="Tarifa de hotel", domain="[('hotel_id', '=', hotel_id)]")
    tipo_tarifario = fields.Selection([("ip", "IP"), ("conf", "CONF FIT"), ("fit", "Rack")], string="Tarifario", default="ip", required=True)
    ...
    @api.depends("cantidad_noches", "cantidad_habitaciones", "hotel_precio_noche_usd", "reserva_id.moneda")
    ...
    def _onchange_hotel_tarifa_id(self):
        ...
        for record in self:
            if record.hotel_tarifa_id:
                record.fecha_check_in = record.fecha_check_in or record.reserva_id.fecha_inicio or record.reserva_id.fecha_viaje or record.hotel_tarifa_id.fecha_desde
    ...
    def write(self, vals):
        for record in self:
            ...
            super(IncasReservaHotelLinea, record).write(values)
        return True
